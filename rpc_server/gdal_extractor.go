...
func Extract(pb.Request) (*pb.GDALFile, error) {
...
func main() {
	C.GDALAllRegister()
	filePath := os.Args[1]
	gFile, err := Extract(pb.Request{FilePath: filePath})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Problem extracting metadata on file %s: %v", filePath, err)
	}
	outb, err := proto.Marshal(gFile)
	fmt.Fprintf(os.Stdout, string(outb.Bytes()))
}
